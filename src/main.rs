...
enum Commands {
    ...
    /// Capture and symbolize in one go (no file I/O)
    Onephase {
        /// Process ID to attach to
        #[arg(short, long)]
        pid: i32,
...
fn print_x86_64_registers(regs: &user_regs_struct) {
    ...
    println!(
        "    RIP: 0x{:016x}  EFLAGS: 0x{:016x}",
        regs.rip, regs.eflags
    );

    // Segment registers
    println!(
        "    CS: 0x{:04x}  SS: 0x{:04x}  DS: 0x{:04x}  ES: 0x{:04x}  FS: 0x{:04x}  GS: 0x{:04x}",
...
fn print_arm64_registers(regs: &user_regs_struct) {
    ...
    // Special registers
    println!(
        "    SP:  0x{:016x}  PC:  0x{:016x}  PSTATE: 0x{:016x}",
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command {
        Commands::Capture { pid, output } => capture_phase(pid, output),
        Commands::Symbolize { input, executable } => symbolize_phase(input, executable),
        Commands::Onephase { pid } => capture_and_symbolize(pid),
    }
}
...
fn capture_phase(pid: i32, output_path: PathBuf) -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "  stacker symbolize --input {} --executable /path/to/executable",
        output_path.display()
    );
...
fn symbolize_phase(
    ...
    symbolize_from_capture_data(&capture_data, &executable_path)
}

fn symbolize_from_capture_data(
    ...
    let mut symbolizer = Symbolizer::new_from_data(
        &executable_path.to_string_lossy(),
        &capture_data.memory_maps,
        &capture_data.file_base_addresses,
    )?;

    // Symbolize each thread
    for (i, captured_thread) in capture_data.threads.iter().enumerate() {
        println!(
            "\n=== Thread {} (TID: {}, Name: '{}') ===",
            i + 1,
            captured_thread.tid,
            captured_thread.thread_name
        );
...
fn capture_and_symbolize(pid: i32) -> Result<(), Box<dyn std::error::Error>> {
    println!("Stacker v0.1.0 - Multi-architecture stack tracer");
    println!("Target architecture: {}", get_architecture_info());
    println!("=== CAPTURE PHASE ===");
    println!("Attaching to process {pid}");
    ...
    // Step 7: Create capture data structure (in-memory only)
    let capture_data = CaptureData {
        timestamp: Utc::now(),
        architecture: get_architecture_info().to_string(),
        process_id: pid,
    ...
    // Now run symbolization directly without persisting
    println!("=== SYMBOLIZE PHASE ===");
    println!("Loaded capture from: {}", capture_data.timestamp);
    println!("Target architecture: {}", capture_data.architecture);
    println!("Original PID: {}", capture_data.process_id);
    ...
    let exe_pathbuf = PathBuf::from(executable_path);
    symbolize_from_capture_data(&capture_data, &exe_pathbuf)
}
...
fn capture_all_threads(
    pid: i32,
    thread_ids: Vec<i32>,
) -> Result<Vec<ThreadInfo>, Box<dyn std::error::Error>> {
...
fn capture_thread_stack_trace(
    main_pid: i32,
    tid: i32,
) -> Result<ThreadInfo, Box<dyn std::error::Error>> {
